...
const photographyHoverCovers={"gulfood-2025":"gulfood.jpg","dubai-airshow-2025":"dubai-airshow.jpg","dubai-boat-show-2025":"dubai-boat-show.jpg","dubai-autodrome-24h-series-2025":"dubai-autodrome-24h.jpg","airlink-photoshoots-2025":"airlink-photoshoots.jpg"};
...
export default function PortfolioHome(){
  ...
  return <>
    ...
    <section className="selected-work section-pad shell"><p className="eyebrow">Selected work</p><div className="selected-work-list">{selectedProjects.map((project,index)=>{const hoverCover=photographyHoverCovers[project.slug];return <Reveal className={`selected-work-row ${hoverCover?"selected-work-photo-fade":""}`} key={project.slug}><Link to={`/project/${project.slug}`}><span className="work-index">{String(index+1).padStart(2,"0")}</span><h3>{project.title}</h3><p>{project.category}</p><time>{project.year}</time><div className="work-thumbs count-1"><img src={project.image} alt={`${project.title} cover`}/>{hoverCover&&<img className="work-thumb-hover" src={publicAsset(`assets/photography/hover-covers/${hoverCover}`)} alt="" aria-hidden="true"/>}</div></Link></Reveal>})}</div><Link className="button button-red" to="/project">View all projects</Link></section>
